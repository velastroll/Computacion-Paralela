...
int main(int argc, char *argv[]) {
	...
	error = cudaMalloc((void**) &d_layer_copy,(float) layer_size*sizeof(float));
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	error = cudaMalloc((void**) &d_posiciones,(int) num_storms*sizeof(int));
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	error = cudaMalloc((void**) &d_maximos,(float) num_storms*sizeof(float));
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	...
	for( i=0; i<num_storms; i++) {
		...
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
		error = cudaMemcpy(d_layer_copy, layer_copy, layer_size * sizeof(float), cudaMemcpyHostToDevice );
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
		...
		for( j=0; j<storms[i].size; j++ ) {
			...
			/* Función del kernel actualiza */
			gpuFunc_actualiza<<<nBlocks, nThreads>>>(d_layer, posicion, energia,layer_size);
		}

		/* 4.2. Relajacion entre tormentas de particulas */
		/* 4.2.1. Copiar valores a capa auxiliar */
		/* Función del kernel copiarLayer */
		gpuFunc_copiarLayer<<<nBlocks, nThreads>>>(d_layer, d_layer_copy);

		/* 4.2.2. Actualizar capa, menos los extremos, usando valores del array auxiliar */
		/* Función del kernel extremos */
		gpuFunc_extremos<<<nBlocks, nThreads>>>(d_layer, d_layer_copy, layer_size);


		/* 4.3. Localizar maximo */
		/*error = cudaMemcpy(d_maximos, maximos, num_storms*sizeof(float), cudaMemcpyHostToDevice );
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
		error = cudaMemcpy(d_posiciones, posiciones, num_storms*sizeof(int), cudaMemcpyHostToDevice );
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );*/

		/* Función del kernel maximo */
	//	gpuFunc_maximos<<<nThreads, nBlocks>>>(d_layer, d_posiciones, d_maximos, layer_size, i);
	error = cudaMemcpy(layer_copy, d_layer_copy, layer_size*sizeof(float), cudaMemcpyDeviceToHost );
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	...
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
		...
		error = cudaMemcpy(layer, d_layer, layer_size*sizeof(float), cudaMemcpyDeviceToHost );
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
		error = cudaMemcpy(maximos, d_maximos, num_storms*sizeof(float), cudaMemcpyDeviceToHost );
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
		error = cudaMemcpy(posiciones, d_posiciones, num_storms*sizeof(int), cudaMemcpyDeviceToHost );
		if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );*/

	}
	error = cudaFree(d_layer);
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	error = cudaFree(d_layer_copy);
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	error = cudaFree(d_maximos);
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
	error = cudaFree(d_posiciones);
	if ( error != cudaSuccess ) printf("ErrCUDA: %s\n", cudaGetErrorString( error ) );
