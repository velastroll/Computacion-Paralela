 ...
 */
 ...
 int main(int argc, char *argv[]) {
	...
	int BLOQUES = ceil(layer_size/HILOS);
	...
	for( i=0; i<num_storms; i++) {
		...
		for( j=0; j<storms[i].size; j++ ) {
			...
			gpu_Actualizar<<<BLOQUES, HILOS>>>(layer_device, posicion, energia,layer_size);
		}
		// necesario sincronizar los hilos antes de la siguiente iteracion
		// cudaDeviceSynchronize();
		
		// Hacemos una copia de la capa en DEVICE
		gpu_Copiar<<<BLOQUES, HILOS>>>(layer_device, layer_device_copy,layer_size);
